...
void SwapLedChannel(LedChannel channel, int led_strip_index) {
  ...
  uint8_t int_pin =
      (led_strip_index == 0) ? PIN_LED_EXT_CTRL_1 : PIN_LED_EXT_CTRL_2;
...
void PlayLedEffect(uint8_t led_strip_index) {
  ...
  case 2: // Gradient Wave
    SwapLedChannel(LedChannel::Internal, led_strip_index);
    SetGradientWave(led_strip_index, settings.num_leds, settings.start_color,
                    settings.end_color, settings.speed);
    break;
  case 3: // Gradient Moving
    SwapLedChannel(LedChannel::Internal, led_strip_index);
    SetMovingGradient(led_strip_index, settings.num_leds, settings.start_color,
                      settings.end_color, settings.speed);
    break;
  case 4: // Rainbow
    SwapLedChannel(LedChannel::Internal, led_strip_index);
...
void SetGradientWave(uint8_t led_strip_index, uint8_t num_leds,
                     int32_t start_color, int32_t end_color, uint8_t speed) {
  unsigned long current_time = millis();
  float time_factor =
      current_time *
      (speed * 0.00001f); // Adjust multiplier for desired speed range
...
void SetMovingGradient(uint8_t led_strip_index, uint8_t num_leds,
                       int32_t start_color, int32_t end_color, uint8_t speed) {
  unsigned long current_time = millis();
  float offset =
      fmod(current_time * speed * 0.00001f, 1.0f); // Adjust multiplier

  for (int i = 0; i < num_leds; i++) {
    float position = static_cast<float>(i) / (num_leds - 1) + offset;
    if (position > 1.0f)
      position -= 1.0f;
    a_LedBuffers[led_strip_index][i] =
        InterpolateColor(start_color, end_color, position);
  }
}

void SetRainbowWave(uint8_t led_strip_index, int num_leds, uint8_t speed,
                    uint8_t delta_hue) {
